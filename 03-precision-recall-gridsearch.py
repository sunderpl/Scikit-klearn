...
def answer_two():
    ...
    return dummy_majority.score(X_test, y_test), recall_score(y_test,y_predict)
...
def answer_four():
    ...
    # #Set a threshold -220
    y_predict = np.where(y_score > -220, 1, 0)
...
def answer_five():
    ...
    i = np.argmin(p<0.75)
    # templist = list(zip(p,r,t))
    # templist[i]
    # r[i]

# no exact 0.16 for roc curve, but tpr nearfpr=0.16 is constant
    fpr, tpr, t = roc_curve(y_test, y_score)
    j = np.argmin(fpr<0.16)
    # templist = list(zip(fpr,tpr,t))
    # templist[j]
    
    return r[i],tpr[j] # Return your answer
